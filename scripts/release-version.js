...
function setPackageVersion(packageJsonPath, version) {
    try {
        ...
        coloredOutput(`Error updating ${packageJsonPath}: ${error.message}`, 'red', emojis.error);
        return false;
    }
}
...
function updateEnvFile(envFilePath, version) {
    try {
        ...
        coloredOutput(`Error updating .env file: ${error.message}`, 'red', emojis.error);
        return false;
    }
}
...
async function main() {
    ...
    if (!fs.existsSync(packageJsonPath)) {
        coloredOutput(`package.json not found in ${packageJsonPath}`, 'red', emojis.error);
        coloredOutput('Please run this script from the project root directory', 'yellow', emojis.info);
        process.exit(1);
    }
    ...
    if (gitStatus && gitStatus.trim() && !options.force) {
        ...
        console.log('');
        const continueAnswer = await promptUser('Do you want to continue anyway? (y/N): ');
    ...
    coloredOutput(`Current version in package.json: ${currentVersion}`, 'magenta', emojis.info);
    ...
    try {
        const tagOutput = execCommand('git tag -l --sort=-version:refname "v*"', { silent: true });
            ...
            if (selectedTag) {
                ...
                if (cleanVersion.match(/^[0-9]+\.[0-9]+\.[0-9]+$/)) {
                    ...
                    if (selectedTag.includes('-')) {
                        coloredOutput(`Last stable Git tag: ${selectedTag} (using clean version: ${lastVersion})`, 'magenta', emojis.info);
                    } else {
                        coloredOutput(`Last Git tag: ${selectedTag} (version: ${lastVersion})`, 'magenta', emojis.info);
                    }
                } else {
                    coloredOutput(`Last Git tag: ${selectedTag} (invalid format, using ${lastVersion})`, 'yellow', emojis.info);
                }
            } else {
                coloredOutput(`No previous Git tags found, starting from ${lastVersion}`, 'magenta', emojis.info);
            }
        } else {
            coloredOutput(`No previous Git tags found, starting from ${lastVersion}`, 'magenta', emojis.info);
        }
    } catch (error) {
        coloredOutput(`Could not read Git tags, starting from ${lastVersion}`, 'yellow', emojis.info);
    }
    ...
    if (currentVersion !== '0.0.0') {
        ...
        // Compare versions
        const isCurrentHigher = currentParts[0] > lastParts[0] || 
                              (currentParts[0] === lastParts[0] && currentParts[1] > lastParts[1]) ||
                              (currentParts[0] === lastParts[0] && currentParts[1] === lastParts[1] && currentParts[2] > lastParts[2]);
        
        if (isCurrentHigher) {
            baseVersion = currentVersion;
            coloredOutput(`Using package.json version ${currentVersion} as base (higher than Git tag)`, 'green', emojis.info);
        } else {
            coloredOutput(`Using Git tag version ${lastVersion} as base (higher than package.json)`, 'green', emojis.info);
        }
    } else {
        coloredOutput(`Using Git tag version ${lastVersion} as base`, 'green', emojis.info);
    }
    ...
    if (options.version) {
        if (options.version.match(/^[0-9]+\.[0-9]+\.[0-9]+$/)) {
            ...
            coloredOutput(`Using provided version: ${newVersion}`, 'green', emojis.success);
        } else {
            coloredOutput(`Invalid version format: ${options.version}`, 'red', emojis.error);
            coloredOutput('Please use semantic versioning (e.g., 1.2.3)', 'yellow', emojis.info);
            process.exit(1);
        }
    } else if (options.type) {
        newVersion = incrementVersion(baseVersion, options.type);
        coloredOutput(`Using ${options.type} increment: ${newVersion}`, 'green', emojis.success);
    } else {
        ...
        console.log('');
        coloredOutput(`Version suggestions based on current version (${baseVersion}):`, 'cyan', emojis.gear);
        ...
        while (!validInput) {
            const versionInput = await promptUser('Select version type (1-4) or enter custom version: ');
            ...
            switch (versionInput) {
                ...
                case '3':
                    ...
                    break;
                case '4':
                    const customVersion = await promptUser('Enter custom version (e.g., 2.1.0): ');
                    if (customVersion.match(/^[0-9]+\.[0-9]+\.[0-9]+$/)) {
                        newVersion = customVersion;
                        validInput = true;
                    } else {
                        coloredOutput('Invalid version format. Please use semantic versioning (e.g., 1.2.3)', 'red', emojis.error);
                    }
                    break;
                default:
                    ...
                    if (versionInput.match(/^[0-9]+\.[0-9]+\.[0-9]+$/)) {
                        ...
                        coloredOutput('Invalid input. Please select 1-4 or enter a valid version', 'red', emojis.error);
                    }
            }
        }
    }
    ...
    // Dry run mode
    if (options.dryRun) {
        coloredOutput('DRY RUN MODE - No changes will be made', 'yellow', emojis.info);
        console.log('');
        coloredOutput('Would update:', 'cyan', emojis.info);
        console.log(`  • package.json version to ${newVersion}`);
        console.log(`  • .env file with new version`);
    ...
    console.log(`  • Update .env file with new version`);
    ...
    if (setPackageVersion(packageJsonPath, newVersion)) {
        coloredOutput(`Updated package.json version to ${newVersion}`, 'green', emojis.success);
    } else {
    ...
    if (updateEnvFile(envFilePath, newVersion)) {
        ...
        if (envExists) {
            coloredOutput(`Updated .env file with VERSION=${newVersion}`, 'green', emojis.success);
        } else {
            coloredOutput(`Created new .env file with VERSION=${newVersion}`, 'green', emojis.success);
        }
    } else {
    ...
    const releasePackageJsonPath = path.join(appDir, 'release', 'app', 'package.json');
    if (fs.existsSync(releasePackageJsonPath)) {
        ...
        if (setPackageVersion(releasePackageJsonPath, newVersion)) {
            coloredOutput(`Updated release/app/package.json version to ${newVersion}`, 'green', emojis.success);
        }
    }
...
sudo apt-get install -f`;
    ...
    console.log('  • Azure DevOps: https://dev.azure.com/your-org/MthBdeIotClient/_build');
    console.log('  • GitHub Releases: https://github.com/mthitservice/MTHBDEIOTClient/releases');
    console.log('  • Latest Release: https://github.com/mthitservice/MTHBDEIOTClient/releases/latest');
    console.log('');
    coloredOutput(`Version ${newVersion} released successfully!`, 'green', emojis.success);
}
